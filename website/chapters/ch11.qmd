...
![Dari teks mentah ke matriks sparse](../figures/ch11-fig-1.png){#fig-ch11-fig-1}
...
### Sublinear TF dan BM25 {.pendalaman-title .unnumbered .unlisted}
...
![Ruang sparse dan ruang embedding padat](../figures/ch11-fig-2.png){#fig-ch11-fig-2}
...
### Subword untuk morfologi, dan embedding Matryoshka {.pendalaman-title .unnumbered .unlisted}
...
![Dari dokumen ke vektor kalimat atau dokumen](../figures/ch11-fig-3.png){#fig-ch11-fig-3}
...
| Representasi | Menangkap apa | Dimensibentuk | Kekuatan | Batas utama |
| --- | --- | --- | --- | --- |
| BoWn-gram | Kehadiran token dan urutan lokal pendek | Sparse, sebesar vocabulary | Kuat sebagai *baseline*, mudah dijelaskan | Buta konteks dan sinonim |
| TF-IDF | Token yang khas bagi dokumen | Sparse berbobot | Kuat untuk klasifikasisearch kecil-menengah | Tidak memahami makna dan sinonim |
| Static word embedding | Kedekatan semantik token | Dense per token | Sinonim lebih dekat | Satu vektor untuk semua konteks |
| Contextual token embedding | Makna token dalam konteks | Dense per tokensubword | Menangani polisemi | Perlu model besar dan pooling |
| Sentencedocument embedding | Makna teks sebagai satu vektor | Dense per teks | Efisien untuk retrievalklasifikasi | Pooling dan panjang dokumen menjadi isu |

: Tangga representasi teks {#tbl-ch11-8}
...
### Memilih ekstraktor, MTEB dan ekosistem modern {.pendalaman-title .unnumbered .unlisted}
...
![Ekstraksi beku dan fine-tuning](../figures/ch11-fig-4.png){#fig-ch11-fig-4}
...
| Situasi | Cenderung ke ekstraksi beku | Cenderung ke fine-tuning | Perhatian |
| --- | --- | --- | --- |
| Dataset berlabel kecil | Ya | Tidak, kecuali adaptasi ringan | Risiko overfitting tinggi |
| Dataset in-domain besar | Bisa sebagai *baseline* | Ya | Butuh validasi dan compute |
| Latensi ketat | Ya, embedding dapat di-cache | Mungkin mahal | Ukur inference end-to-end |
| Kesenjangan domain kuat | *Baseline* perlu diuji | Ya, jika label cukup | Jargon dapat mengubah makna |
| Butuh *baseline* interpretable | TF-IDF fitur sparse lebih mudah | Tidak utama | Bandingkan dengan *baseline* klasik |

: Feature extraction vs fine-tuning {#tbl-ch11-9}
