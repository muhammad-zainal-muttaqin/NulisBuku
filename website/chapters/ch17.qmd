...
```{mermaid}
%%| label: fig-ch17-17_1
%%| fig-cap: "Monitoring Drift Fitur (Data Drift Diagram) Pasca-Produksi"
...
Dalam persamaan tersebut, $k$ adalah jumlah kelompok (*bin*) dari variabel yang dievaluasi, $P_{aktual, i}$ adalah proporsi sampel pada *bin* ke-$i$ di data aktual (*inference*), dan $P_{ekspektasi, i}$ adalah proporsi sampel pada *bin* yang sama di data yang diharapkan (pelatihan).
...
*Training-serving skew* merupakan ancaman yang paling sulit terdeteksi karena bersifat *silent failure* (gagal tanpa suara). Kode tidak menghasilkan galat atau pesan *error*, namun akurasi probabilitasnya merosot drastis. Fenomena ini sering dipicu oleh hal-hal operasional:
...
```{mermaid}
%%| label: fig-ch17-17_2
%%| fig-cap: "Arsitektur Feature Store (Feast) Offline dan Online Store"
%%{init: {'theme': 'neutral', 'themeVariables': { 'edgeLabelBackground': '#ffffff' }}}%%
graph TD
...
Jika tumpukan data harian melanggar batasan validasi ini, proses akan ditolak (*rejected*) dan peringatan diaktifkan. Tanpa palang pintu ini, nilai pengamatan cacat akan mencemari *pipeline* dan membuat model menghasilkan prediksi yang keliru.
...
```{mermaid}
%%| label: fig-ch17-17_4
%%| fig-cap: "Jalur Ekstraksi Fitur Batch vs Streaming Features"
%%{init: {'theme': 'neutral', 'themeVariables': { 'edgeLabelBackground': '#ffffff' }}}%%
graph TD
...
Dalam persamaan tersebut, $k$ adalah jumlah partisi (*bin*) dari suatu fitur, $P_{inference,i}$ adalah proporsi sampel pada *bin* ke-$i$ di lingkungan produksi, dan $P_{train,i}$ adalah proporsi sampel pada *bin* yang sama di set pelatihan.
...
Dalam persamaan tersebut, $k$ adalah jumlah kategori atau *bin* pembagian fitur numerik, $A_i$ adalah proporsi observasi pada *bin* $i$ saat produksi (*inference*), dan $E_i$ adalah proporsi observasi pada *bin* yang sama saat pelatihan (*training*).
