...
Ketiadaan data dan kehadiran pencilan (*outlier*) merupakan kenyataan empiris yang tidak terhindarkan saat kita bekerja dengan data mentah di dunia nyata. Algoritma pemelajaran mesin tidak dapat menoleransi nilai kosong pada saat kalkulasi gradien atau jarak, sedangkan pencilan ekstrem yang menyimpang jauh dari pusat distribusi dapat dengan mudah mendistorsi estimasi parameter model. Sayangnya, tindakan pembersihan secara manual dan baris-per-baris sering kali merusak variansi asli, menyembunyikan sinyal anomali berharga, dan yang paling berbahaya, memicu kebocoran data (*data leakage*) jika parameter pengisi dihitung sebelum pemisahan data (*data split*) dilakukan.

Bab ini meletakkan fondasi bagaimana merancang penanganan nilai kosong dan pencilan secara sistematis, *reproducible*, dan berbasis *pipeline*. Kita mendalami mekanisme hilangnya data (MCAR, MAR, MNAR) sebagai asupan diagnostik sebelum otomatisasi, membedakan imputasi sederhana dari imputasi iteratif multivariat berbasis model, menggunakan status kekosongan sebagai fitur baru (*missing indicator*), serta mendeteksi dan mentransformasi pencilan menggunakan parameter statistik yang tangguh (*robust*). Seluruh proses ini dikemas ke dalam alur kerja terstruktur untuk menjamin integritas data dari tahap pelatihan hingga inferensi di produksi.
