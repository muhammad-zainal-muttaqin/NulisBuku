...
Ketiadaan data dan kehadiran pencilan (\emph{outlier}) merupakan kenyataan empiris yang tidak terhindarkan saat kita bekerja dengan data mentah di dunia nyata. Algoritma pemelajaran mesin tidak dapat menoleransi nilai kosong pada saat kalkulasi gradien atau jarak, sedangkan pencilan ekstrem yang menyimpang jauh dari pusat distribusi dapat dengan mudah mendistorsi estimasi parameter model. Sayangnya, tindakan pembersihan secara manual dan baris-per-baris sering kali merusak variansi asli, menyembunyikan sinyal anomali berharga, dan yang paling berbahaya, memicu kebocoran data (\emph{data leakage}) jika parameter pengisi dihitung sebelum pemisahan data (\emph{data split}) dilakukan.

Bab ini meletakkan fondasi bagaimana merancang penanganan nilai kosong dan pencilan secara sistematis, \emph{reproducible}, dan berbasis \emph{pipeline}. Kita mendalami mekanisme hilangnya data (MCAR, MAR, MNAR) sebagai asupan diagnostik sebelum otomatisasi, membedakan imputasi sederhana dari imputasi iteratif multivariat berbasis model, menggunakan status kekosongan sebagai fitur baru (\emph{missing indicator}), serta mendeteksi dan mentransformasi pencilan menggunakan parameter statistik yang tangguh (\emph{robust}). Seluruh proses ini dikemas ke dalam alur kerja terstruktur untuk menjamin integritas data dari tahap pelatihan hingga inferensi di produksi.
...
\textbf{Strategi Pengisian Nilai Kosong.} Diagnosis pola kekosongan terlebih dahulu sebelum memilih metode. Pada skenario MCAR, imputasi univariat sederhana (rata-rata/median) cukup memadai dan efisien. Jika korelasi antar-variabel kuat (MAR), terapkan imputasi multivariat (seperti KNN atau MICE) agar struktur distribusi asli tetap terjaga. Pada kondisi MNAR di mana ketiadaan data menyimpan informasi penting, pertahankan sinyal tersebut dengan mengonstruksi fitur biner pendukung berupa \emph{missing indicator}.
