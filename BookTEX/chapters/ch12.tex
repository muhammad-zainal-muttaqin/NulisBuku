% ==========================================================================
% ch12.tex — dihasilkan dari v2/drafts oleh tools/md2tex.py
% Sumber: v2\drafts\ch12\ch12_draft_v2.md
% Boleh disunting manual; menjalankan md2tex.py lagi akan menimpa.
% ==========================================================================
\chapter{Citra dan Audio}\label{citra-dan-audio}
...
Bab ini membahas dua jalur representasi untuk citra dan audio. Jalur pertama adalah representasi yang dirancang manusia, seperti HOG untuk citra serta spectrogram, MFCC, dan chroma untuk audio. Jalur kedua adalah representasi yang dipelajari mesin, seperti CNN, Vision Transformer, dan encoder audio pratelatih yang belajar langsung dari waveform. Bab ini menguraikan penskalaan kanal, augmentasi sebagai keputusan representasi, embedding citra dan audio, serta penggabungan (\emph{pooling}) frame berdurasi variabel menjadi vektor berukuran tetap. Tujuannya memahami kapan suatu representasi cukup dan bagaimana validasinya dijaga tetap jujur.

\section{Penskalaan Kanal dan Augmentasi Citra}\label{penskalaan-kanal-dan-augmentasi-citra}

Citra yang tampak utuh bagi manusia masuk ke model sebagai tensor angka. Tahap pertama adalah menyepakati bentuk tensor dan skala nilainya. Citra digital biasanya direpresentasikan dengan dimensi tinggi, lebar, dan kanal. Pada citra RGB, satu piksel mempunyai tiga nilai, yaitu merah, hijau, dan biru. Jika citra berukuran 224 x 224, bentuknya dapat ditulis sebagai 224 x 224 x 3 pada konvensi \emph{channels-last} seperti TensorFlow/Keras, atau 3 x 224 x 224 pada konvensi \emph{channels-first} seperti PyTorch. Nilai piksel sering datang sebagai bilangan 0 sampai 255, lalu diubah menjadi rentang seperti 0 sampai 1 sebelum masuk ke model.
...
Dalam rumus tersebut, \(\mu_c\) adalah rata-rata kanal dan \(\sigma_c\) adalah \emph{standard deviation} kanal. Untuk model yang dilatih dari awal, statistik ini dihitung dari \emph{training split} saja. Model \emph{pretrained} menggunakan statistik yang didokumentasikan khusus untuk bobotnya. Sebagai contoh, sebagian bobot \emph{vision pretrained} yang dilatih pada ImageNet mendokumentasikan \emph{mean} RGB {[}0.485, 0.456, 0.406{]} beserta \emph{standard deviation} tertentu. Angka tersebut tidak berlaku untuk semua model \emph{vision pretrained}. Mengganti statistik bawaan dengan statistik \emph{dataset} lokal juga tidak selalu benar, karena lapisan awal model sudah belajar membaca \emph{input} dengan konvensi lama.

Gambar 12.1 memperlihatkan perbedaan antara jalur pelatihan dan jalur evaluasi. Jalur pelatihan boleh memakai augmentasi acak, sedangkan \emph{preprocessing} validasi dan \emph{test} secara default dibuat deterministik agar evaluasi dapat direproduksi. \emph{Test-time augmentation} yang ditetapkan sebelumnya tetap sah jika transformasi dan aturan penggabungan prediksinya reproducible serta sesuai dengan prosedur \emph{serving}.

\begin{figure}[tbp]\centering
\includegraphics[width=111.6mm]{ch12-fig-1}
\caption{Tensor citra, augmentasi pelatihan, dan preprocessing evaluasi}
...
Augmentasi membuat contoh pelatihan baru melalui transformasi yang dianggap tidak mengubah label, misalnya crop, flip, rotasi kecil, color jitter, blur, atau \emph{noise}. Pada Fashion-MNIST, rotasi kecil atau translasi ringan dapat membantu jika label pakaian tetap terbaca. Crop tidak selalu aman. Jika gambar ankle boot dipotong sampai sol atau bentuk utamanya hilang, label lama menjadi meragukan. Pada audio digit FSDD, time shift atau \emph{noise} ringan dapat aman, tetapi transformasi yang memotong awal ucapan dapat menghapus informasi digit.

Dengan kata lain, augmentasi bukan data gratis. Augmentasi adalah pernyataan bahwa label tetap benar di bawah transformasi tertentu. Pernyataan tersebut harus sesuai dengan domain, tugas, dan cara data dipakai saat deployment. Setelah input citra dipersiapkan, pertanyaan berikutnya adalah apakah struktur visual perlu ditulis manusia sebagai \emph{descriptor}, atau dibiarkan dipelajari oleh model.

\begin{pendalaman}{Kebijakan augmentasi otomatis dan augmentasi lintas sampel}
...
\begin{figure}[tbp]\centering
\includegraphics[width=111.6mm]{ch12-fig-2}
\caption{Dari patch citra ke descriptor HOG}
...
\section{CNN sebagai Feature Extractor dan Model Vision Pretrained}\label{cnn-sebagai-feature-extractor-dan-model-vision-pretrained}
...
Dalam rumus tersebut, \(H\) dan \(W\) adalah tinggi dan lebar citra, sedangkan \(P\) adalah ukuran sisi patch. Rumus ini mengasumsikan \(H\) dan \(W\) habis dibagi \(P\); jika tidak, citra perlu di-\emph{resize}, di-\emph{pad}, atau di-\emph{crop} sesuai aturan model. Citra 224 x 224 dengan patch 16 x 16 menghasilkan \(224 \times 224 / 16^2 = 196\) token patch. Angka ini penting karena \emph{patch size} langsung memengaruhi resolusi lokal dan biaya komputasi.
...
\begin{figure}[tbp]\centering
\includegraphics[width=111.6mm]{ch12-fig-4}
\caption{Citra sebagai urutan patch embedding}
\label{fig:ch12-fig-4}
\end{figure}
...
\begin{pendalaman}{Ekosistem ekstraktor visual pretrained}
...
\section{Audio dari Waveform, Spectrogram, MFCC, dan Chroma}\label{audio-dari-waveform-spectrogram-mfcc-dan-chroma}

Pada sisi audio, sinyal yang terdengar sebagai digit lisan pertama-tama hanyalah \emph{waveform}, yaitu amplitudo sebagai fungsi waktu. Setiap saat hanya memuat satu atau beberapa nilai, bergantung pada jumlah kanal. Semua sumber suara sudah tercampur di sana, mulai dari ucapan, musik, langkah kaki, mesin, ruangan, sampai \emph{noise}. Model yang menerima \emph{waveform} mentah harus belajar sendiri struktur frekuensi, ritme, dan pola temporal.
...
MFCC, atau \emph{Mel-frequency cepstral coefficients}, melangkah lebih jauh. Representasi ini merangkum bentuk envelope spektral menjadi koefisien yang lebih ringkas setelah mentransformasikan energi per filterbank mel melalui \emph{discrete cosine transform} (DCT). DCT mendekorelasikan energi filterbank mel secara aproksimatif, tetapi tidak menjamin koefisien MFCC tidak berkorelasi secara statistik pada setiap \emph{dataset}. Secara historis, MFCC sangat kuat untuk tugas speech dan tetap berguna pada dataset kecil atau edge deployment yang membutuhkan fitur padat dan murah. Chroma berbeda lagi. Representasi ini merangkum energi ke dua belas pitch class, dengan octave yang dilipat. Karena itu, chroma lebih cocok untuk musik, chord, genre, atau tugas yang bergantung pada harmoni.
...
\begin{figure}[tbp]\centering
\includegraphics[width=111.6mm]{ch12-fig-5}
\caption{Tangga representasi audio klasik}
\label{fig:ch12-fig-5}
\end{figure}
...
\section{Encoder Audio Pretrained Berbasis Raw Waveform}\label{encoder-audio-pretrained-berbasis-raw-waveform}
...
Resep self-supervised learning pada audio dapat dijelaskan sederhana. Model melihat representasi audio yang sebagian segmennya ditutup atau di-\emph{mask}, lalu dilatih memprediksi bagian yang hilang atau unit target tertentu. Dengan latihan seperti ini, model dipaksa menyerap struktur fonetik dan akustik tanpa membutuhkan label manual untuk setiap potongan suara.
...
Perpindahan ini adalah titik penting menuju representasi yang dipelajari mesin pada audio. Model tidak lagi hanya menerima \emph{spectrogram} yang dirancang manusia, tetapi belajar struktur dari \emph{waveform}. Meski begitu, \emph{compute}, \emph{latency}, dan \emph{domain mismatch} tetap nyata. Untuk \emph{environmental sound}, musik, atau suara mesin, model \emph{pretrained speech} belum tentu pilihan terbaik. Pendekatan berbasis \emph{spectrogram} atau encoder \emph{pretrained} khusus \emph{non-speech} bisa lebih sesuai. Setelah encoder dipilih, rangkaian frame atau \emph{chunk} perlu diubah menjadi representasi berukuran tetap.

\begin{pendalaman}{Peta ekstraktor audio pretrained}
...
\section{Audio Embedding dan Agregasi Sepanjang Waktu}\label{audio-embedding-dan-agregasi-sepanjang-waktu}
...
Cara paling sederhana adalah average pooling. Semua frame vector dirata-ratakan menjadi satu clip vector. Ini cocok ketika informasi digit tersebar cukup merata sepanjang klip pendek. Namun, average pooling dapat menghapus bagian penting jika klip didominasi silence. Max pooling menjaga kejadian singkat seperti onset ucapan, tetapi mudah bereaksi terhadap \emph{noise} ekstrem. Attention pooling belajar memilih frame penting, tetapi membutuhkan data dan pelatihan tambahan. Pilihan lain adalah tidak memaksa pooling awal. Sequence model mempertahankan urutan frame agar timing tetap lebih utuh.

Label clip-level juga bisa lemah. Pada FSDD, label digit berlaku untuk satu klip pendek, tetapi silence di awal atau akhir tetap dapat mengencerkan sinyal jika pooling terlalu kasar. Pada audio event yang lebih panjang, masalahnya lebih berat karena peristiwa penting mungkin hanya muncul beberapa detik. Segmentasi atau \emph{chunk-level embedding} membantu ketika lokasi waktu peristiwa penting perlu dipertahankan dan satu vektor global akan mengaburkannya.
...
\tabeljudul{Tabel 12.1 — Peta representasi citra dan audio}
...
\begin{xltabular}{\linewidth}{@{}>{\raggedright\arraybackslash}p{(\linewidth-8\tabcolsep)*\real{0.160}} >{\raggedright\arraybackslash}p{(\linewidth-8\tabcolsep)*\real{0.149}} >{\raggedright\arraybackslash}p{(\linewidth-8\tabcolsep)*\real{0.244}} >{\raggedright\arraybackslash}p{(\linewidth-8\tabcolsep)*\real{0.210}} >{\raggedright\arraybackslash}p{(\linewidth-8\tabcolsep)*\real{0.237}}@{}}
\toprule
...
HOG & Citra & Representasi yang dirancang manusia & Shape cue, baseline klasik & Sensitif pada parameter dan tugas \\
CNN features pretrained & Citra & Representasi yang dipelajari mesin & Klasifikasi citra, transfer learning & Domain mismatch dan preprocessing \\
ViT patch embeddings & Citra & Representasi yang dipelajari mesin & Retrieval, klasifikasi, dense tasks & Biaya komputasi dan bias embedding \\
Spectrogram\slash{}mel & Audio & Representasi yang dirancang manusia & Speech, mesin, environmental sound & Window, hop, dan sampling menentukan sinyal \\
MFCC & Audio & Representasi yang dirancang manusia & Speech kecil, edge deployment & Mengompres banyak detail spektral \\
Chroma & Audio & Representasi yang dirancang manusia & Musik, chord, genre & Kurang cocok untuk non-pitch tasks \\
Raw-waveform encoder embeddings & Audio & Representasi yang dipelajari mesin & Speech\slash{}audio transfer learning & Compute, domain match, pooling \\
\bottomrule
\end{xltabular}\endgroup\par\vspace{5pt}

}

Tabel 12.2 berfokus pada keputusan \emph{pooling} temporal audio. Baris-barisnya membantu memilih strategi berdasarkan apakah peristiwa tersebar merata, muncul singkat, atau membutuhkan urutan.

\tabeljudul{Tabel 12.2 — Strategi pooling temporal audio}
...
\scriptsize\linespread{1}\selectfont
\renewcommand{\_}{\textunderscore\hspace{0pt}}%
\begin{xltabular}{\linewidth}{@{}>{\raggedright\arraybackslash}p{(\linewidth-4\tabcolsep)*\real{0.170}} >{\raggedright\arraybackslash}p{(\linewidth-4\tabcolsep)*\real{0.467}} >{\raggedright\arraybackslash}p{(\linewidth-4\tabcolsep)*\real{0.363}}@{}}
\toprule
...
Average pooling & Tekstur suara stasioner, misalnya machine hum & Peristiwa singkat hilang dalam rata-rata \\
Max pooling & Peristiwa impulsif, misalnya kaca pecah & Sensitif pada \emph{noise} ekstrem \\
Attention pooling & Klip dengan bagian penting yang tidak selalu sama & Butuh label dan pelatihan cukup \\
Sequence model & Tugas yang membutuhkan urutan waktu & Biaya dan kompleksitas lebih tinggi \\
\bottomrule
\end{xltabular}\endgroup\par\vspace{5pt}
...
\tightlist
\item
  TorchVision --- Models and pre-trained weights --- \url{https://pytorch.org/vision/stable/models.html}. Backbone citra pralatih siap pakai.
\item
  Dosovitskiy dkk. (2020), Vision Transformer --- \url{https://arxiv.org/abs/2010.11929}. Transformer untuk citra (ViT).
\item
  librosa --- \url{https://librosa.org/doc/latest/}. Ekstraksi fitur audio (spektrogram, MFCC).
\item
  Baevski dkk. (2020), wav2vec 2.0 --- \url{https://arxiv.org/abs/2006.11477}. Representasi audio self-supervised.
\item
  Radford dkk. (2021), CLIP --- \url{https://arxiv.org/abs/2103.00020}. Embedding citra--teks bersama.
\item
  Oquab dkk. (2023), DINOv2 --- \url{https://arxiv.org/abs/2304.07193}. Fitur citra self-supervised umum.
\item
  Radford dkk. (2022), Whisper --- \url{https://arxiv.org/abs/2212.04356}. Model pengenalan wicara robust.
\item
  Zhang dkk. (2017), mixup --- \url{https://arxiv.org/abs/1710.09412}. Augmentasi interpolasi untuk pelatihan.
\end{itemize}
