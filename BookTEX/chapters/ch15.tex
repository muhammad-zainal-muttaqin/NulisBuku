...
Di mana \(x_i\) adalah input, \(y_i\) adalah label, \(\phi\) mewakili bobot model \emph{pretrained}, dan \(\theta\) merupakan bobot lapisan klasifikasi. Pendekatan adaptasi ditentukan oleh perlakuan terhadap parameter \(\phi\) selama proses optimasi. Terdapat empat titik utama pada spektrum adaptasi ini:
...
\tightlist
\item
  \(\mathbf{u}\) dan \(\mathbf{v}\) merepresentasikan dua vektor \emph{embedding} berdimensi \(d\) yang diukur tingkat kemiripannya.
\item
  \(\mathbf{u} \cdot \mathbf{v}\) melambangkan \emph{dot product} (perkalian titik) antara kedua vektor.
\item
...
\subsection{Metode Linear Probing}\label{metode-linear-probing}
...
\tightlist
\item
  \(f(x_i)\) adalah representasi yang dipelajari mesin dan parameter asalnya dibiarkan tetap (\emph{frozen}).
\item
...
\subsection{Mengidentifikasi Domain Shift}\label{mengidentifikasi-domain-shift}
...
\tightlist
\item
  \(T_{num}^{(j)}\) adalah \emph{embedding} berwujud vektor padat untuk fitur numerik tersebut.
\item
...
\item
  \(\mathbf{e}_{learned} \in \mathbb{R}^k\) adalah representasi padat yang diekstraksi dari model \emph{pretrained} (misalnya dari representasi \emph{bottleneck} pada ResNet atau ekstrasi token CLS pada arsitektur Transformer).
\item
