% ==========================================================================
% ch09.tex — dihasilkan dari v2/drafts oleh tools/md2tex.py
% Sumber: v2\drafts\ch09\ch09_draft_v2.md
...
\section{Apa yang Membuat Fitur Baik?}\label{apa-yang-membuat-fitur-baik}
...
\tabeljudul{Tabel 9.1 — Dimensi kualitas fitur}
...
\begin{xltabular}{\linewidth}{@{}>{\raggedright\arraybackslash}p{(\linewidth-4\tabcolsep)*\real{0.160}} >{\raggedright\arraybackslash}p{(\linewidth-4\tabcolsep)*\real{0.460}} >{\raggedright\arraybackslash}p{(\linewidth-4\tabcolsep)*\real{0.380}}@{}}
\toprule
...
Availability & Apakah fitur tersedia pada waktu inferensi dan sesuai \emph{cutoff}? & Agregat lambat dua detik untuk model fraud milidetik \\
Stability & Apakah distribusi dan kontribusinya stabil lintas waktu\slash{}populasi? & Fitur berubah karena sistem pencatatan baru \\
...
Robustness & Apakah fitur tahan terhadap \emph{noise}, missingness, dan shift? & Sensor kecil error membuat skor berubah besar \\
...
Gambar 9.1 memperlihatkan tesis bab ini dalam bentuk radar. Sebuah fitur dapat sangat tinggi pada \emph{predictive utility}, tetapi rendah pada \emph{availability} dan \emph{legitimacy}. Fitur seperti itu memerlukan keputusan sadar tentang alternatif, cara mengurangi risiko, dan batas penggunaannya.

\begin{figure}[tbp]\centering
\includegraphics[width=111.6mm]{ch09-fig-1}
\caption{Radar kualitas fitur}
...
\begin{pendalaman}{Memantau stabilitas dengan Population Stability Index}
Drift distribusi fitur dapat dipantau sebelum label produksi tersedia memakai PSI, dengan rumus \(\text{PSI} = \sum_{i=1}^{k} (A_i - E_i) \ln\!\dfrac{A_i}{E_i}\). Dalam hal ini, \(A_i\) adalah proporsi bin produksi dan \(E_i\) adalah proporsi bin pada data training. Dalam praktik, bin dengan \(E_i\) atau \(A_i\) nol perlu ditangani dengan penggabungan bin, \emph{smoothing} kecil, atau konvensi khusus yang dipilih secara konsisten. Secara konvensional, nilai di bawah 0,1 sering dibaca stabil, nilai antara 0,1 dan 0,25 sebagai pergeseran moderat yang perlu pemantauan lebih lanjut, sedangkan nilai di atas 0,25 menandai pergeseran besar yang perlu investigasi atau retraining. PSI adalah konvensi monitoring, bukan uji statistik formal. Ambangnya merupakan folklore yang berguna bila dipakai konsisten.
\end{pendalaman}

\section{\texorpdfstring{Menetapkan \emph{Baseline}}{Menetapkan Baseline}}\label{menetapkan-baseline}

Kualitas fitur selalu membutuhkan pembanding. Tanpa \emph{baseline}, kalimat ``model ini bagus'' tidak memiliki konteks, dan kalimat ``fitur ini penting'' mudah terdengar lebih kuat daripada buktinya. \emph{Baseline} adalah sistem rujukan yang digunakan untuk menilai apakah fitur baru benar-benar memberi manfaat. \emph{Baseline} dapat berupa prediksi kelas mayoritas, model kecil yang mudah dijelaskan, \emph{feature set} produksi sebelumnya, atau kelompok fitur minimal yang sudah disepakati.

Lantai paling sederhana adalah dummy estimator yang mengabaikan fitur sama sekali. Pada regresi, \emph{baseline} dummy dapat memprediksi rata-rata target training untuk semua sampel.

\[
\hat{y}_i = \dfrac{1}{n}\sum_{j=1}^{n} y_j
\]

Dalam rumus tersebut, \(\hat{y}_i\) adalah prediksi untuk sampel ke-\(i\), sedangkan \(y_j\) adalah target training. Untuk klasifikasi, strategi kelas mayoritas atau prevalensi kelas memberi pembanding deterministik yang mengabaikan fitur. Strategi \passthrough{\lstinline!stratified!} dan \passthrough{\lstinline!uniform!} pada \emph{DummyClassifier} adalah kontrol acak, bukan prediktor konstan atau satu \emph{floor} deterministik. Regresi dapat memakai \emph{DummyRegressor} yang memprediksi rata-rata atau median target pelatihan. Setiap \emph{feature set} perlu melewati pembanding yang sesuai dengan tugas dan metriknya; kenaikan kecil dari pembanding sederhana tetap perlu ditimbang terhadap biaya sistem.

\emph{Baseline} juga bisa lebih realistis. Model churn dapat dimulai dari fitur profil pelanggan saja, lalu dibandingkan dengan versi yang menambahkan agregat riwayat transaksi. Model deret waktu dapat dibandingkan dengan \emph{baseline} last observed value sebelum memakai lag dan rolling statistics yang lebih kaya. Dalam semua kasus, \emph{baseline} dan kandidat harus memakai split, metrik, dan protokol validasi yang sama.

Setelah kandidat mengalahkan \emph{baseline}, margin peningkatan perlu dibandingkan dengan harga operasionalnya. Fitur yang menaikkan AUC sedikit tetapi membutuhkan \emph{join} mahal, \emph{storage} besar, dan \emph{monitoring} kompleks mungkin tidak layak. Sebaliknya, fitur sederhana yang memberi kenaikan kecil tetapi stabil dan murah dapat sangat berharga. \emph{Baseline} memberi konteks numerik, sedangkan dimensi kualitas pada Tabel 9.1 memberi konteks keputusan. Langkah berikutnya adalah mencari bagian dari \emph{feature set} yang benar-benar memikul kenaikan tersebut.

\section{\texorpdfstring{\emph{Ablation Study}}{Ablation Study}}\label{ablation-study}

Setelah \emph{baseline} memberi lantai pembanding, ablation membantu mencari bagian mana dari \emph{feature set} yang benar-benar memikul kenaikan performa. Ablation mengukur apa yang terjadi ketika satu fitur atau satu kelompok fitur dihapus atau ditambahkan. Tujuannya bukan mencari importance abstrak, melainkan melihat kontribusi fitur dalam model dan protokol evaluasi tertentu. Jika satu kelompok fitur dihapus dan skor validasi turun besar, kelompok itu memikul informasi penting yang tidak bisa digantikan oleh kelompok lain.
...
Protokolnya sederhana. Latih model dengan \emph{feature set} penuh, hapus satu kelompok fitur, latih ulang dengan protokol yang sama, lalu ukur delta pada validasi. Ulangi untuk kelompok lain dan laporkan variasi lintas fold atau periode waktu bila tersedia. Nyatakan apakah hiperparameter dipertahankan atau disetel ulang pada setiap ablasi. Jika penyetelan ulang merupakan bagian dari prosedur, lakukan hanya di dalam data pelatihan atau \emph{inner fold} untuk setiap kandidat; jika hiperparameter dikunci untuk mengisolasi efek penghapusan fitur, laporkan keputusan itu. Jangan hanya memask kolom setelah model dilatih kecuali memang ingin menguji kerusakan pada model yang sudah ada. Untuk menilai kontribusi \emph{feature set}, model perlu dilatih ulang.
...
\begin{figure}[tbp]\centering
\includegraphics[width=111.6mm]{ch09-fig-2}
\caption{Alur ablation kelompok fitur}
...
\section{\texorpdfstring{\emph{Permutation Importance} dan \emph{Model-Based Importance}}{Permutation Importance dan Model-Based Importance}}\label{permutation-importance-dan-model-based-importance}
...
\begin{figure}[tbp]\centering
\includegraphics[width=111.6mm]{ch09-fig-3}
\caption{Prosedur permutation importance}
...
Tabel 9.2 membandingkan beberapa metode penilaian kontribusi fitur. Tabel ini tidak memilih pemenang tunggal, melainkan membantu mencocokkan alat dengan kebutuhan. \emph{Ablation} lebih tepat untuk kontribusi kelompok, \emph{permutation importance} mengukur penurunan skor saat satu kolom dirusak, sedangkan SHAP memberi penjelasan yang lebih rinci tentang perubahan pada prediksi individu.

\tabeljudul{Tabel 9.2 — Perbandingan metode penilaian kontribusi fitur}
...
\scriptsize\linespread{1}\selectfont
\renewcommand{\_}{\textunderscore\hspace{0pt}}%
\begin{xltabular}{\linewidth}{@{}>{\raggedright\arraybackslash}p{(\linewidth-6\tabcolsep)*\real{0.160}} >{\raggedright\arraybackslash}p{(\linewidth-6\tabcolsep)*\real{0.302}} >{\raggedright\arraybackslash}p{(\linewidth-6\tabcolsep)*\real{0.244}} >{\raggedright\arraybackslash}p{(\linewidth-6\tabcolsep)*\real{0.294}}@{}}
\toprule
...
Impurity\slash{}model-based importance & Kontribusi split atau sinyal internal model & Cepat, tersedia dari estimator & Bias terhadap kardinalitas dan struktur model \\
...
Dalam rumus tersebut, \(f(x)\) adalah keluaran model untuk sampel \(x\), \(\bar{f}\) adalah keluaran acuan atau \emph{baseline} model, dan \(\phi_j(x)\) adalah kontribusi fitur ke-\(j\) untuk sampel tersebut. Aditivitas berlaku pada skala keluaran yang digunakan \emph{explainer}. Untuk model klasifikasi, skala itu dapat berupa \emph{raw margin} atau log-odds, bukan probabilitas, kecuali \emph{explainer} memang dikonfigurasi pada skala probabilitas. Karena itu, skala keluaran harus disebutkan saat nilai SHAP dilaporkan.
...
\begin{figure}[tbp]\centering
\includegraphics[width=111.6mm]{ch09-fig-4}
\caption{Atribusi lokal untuk satu prediksi kredit}
...
Biaya komputasi dan asumsi SHAP berbeda antar \emph{explainer} dan keluarga model. \emph{Mean absolute SHAP} pada \emph{dataset} kadang dipakai sebagai kriteria penyaringan fitur, termasuk dalam praktik yang menggabungkannya dengan \emph{shadow features} seperti BorutaShap. Keputusan penyaringan ini tetap bergantung pada pilihan model, \emph{explainer}, dan data validasi yang dipakai. SHAP perlu diperlakukan sebagai alat diagnosis, bukan standar universal. Kontribusi fitur juga perlu diuji ulang pada \emph{fold}, waktu, dan populasi lain sebelum dipakai sebagai dasar tindakan.

\section{Stabilitas Importance dan Batas Kausalitas}\label{stabilitas-importance-dan-batas-kausalitas}
...
Karena itu, interpretasi \emph{importance} perlu melibatkan \emph{domain review}. Pemeriksaan domain membedakan apakah fitur merupakan sebab, proksi, artefak pencatatan, jalur \emph{leakage}, atau konsekuensi dari target. Bab ini tidak menggantikan metode kausal, tetapi memberi tanda kapan grafik \emph{importance} tidak cukup dipakai sebagai dasar keputusan. Pemeriksaan tersebut penting karena fitur proxy dapat tampak biasa sambil membawa informasi sensitif atau kebijakan yang tidak boleh dipakai sembarangan.

\begin{pendalaman}{Dari prediksi ke intervensi melalui metode kausal}
Jika tujuan analisis adalah intervensi, pertanyaannya perlu dirumuskan sebagai estimand kausal dengan \emph{treatment}, \emph{outcome}, populasi, dan horizon yang jelas. Double atau debiased machine learning serta causal forests dapat membantu mengestimasi efek, tetapi keduanya tetap memerlukan asumsi identifikasi, data dengan overlap yang memadai, dan penanganan confounder yang dapat dipertanggungjawabkan. Metode tersebut tidak mengubah \emph{predictive importance} menjadi efek kausal secara otomatis. Pertanyaan tentang akibat perubahan pada suatu tindakan memerlukan desain dan asumsi kausal, bukan sekadar tambahan \emph{importance chart}.
\end{pendalaman}

\section{Informasi Sensitif dan Fitur Proxy}\label{informasi-sensitif-dan-fitur-proxy}
...
\begin{figure}[tbp]\centering
\includegraphics[width=111.6mm]{ch09-fig-5}
\caption{Jalur proxy dari atribut sensitif ke model}
\label{fig:ch09-fig-5}
\end{figure}

Menghapus atribut terlindungi terlalu awal dapat membuat bias tidak terukur. Audit memerlukan informasi tentang kelompok yang terkena \emph{error} lebih tinggi, \emph{false positive} lebih besar, atau keputusan lebih buruk. Artinya, atribut sensitif kadang perlu dipertahankan dalam lingkungan evaluasi yang aman, meskipun tidak diberikan ke model sebagai fitur.

Prinsip \emph{data minimization} menuntut penilaian apakah setiap fitur memadai, relevan, dan terbatas pada apa yang diperlukan. ``Meningkatkan akurasi'' bukan jawaban otomatis bahwa fitur boleh dipakai. Jika fitur mengandung lokasi sangat presisi, data kesehatan, atau sinyal ekonomi sensitif, manfaat prediktifnya harus dibandingkan dengan kebutuhan tugas dan risiko institusional.
...
Setelah kontribusi, stabilitas, dan risiko proxy diperiksa, keputusan fitur perlu meninggalkan jejak. Tanpa dokumentasi, sebuah kolom seperti \passthrough{\lstinline!skor_aktivitas_terbobot!} dapat hidup bertahun-tahun tanpa ada yang tahu definisinya, sumbernya, alasan pembuatannya, atau batas penggunaannya. Tim baru mengulang eksperimen yang pernah gagal karena tidak ada catatan. Fitur yang dulu valid menjadi usang tanpa ada pemilik yang memeriksa.

Dokumentasi fitur sebaiknya mencatat nama fitur, definisi, sumber tabel, unit, jendela waktu, \emph{cutoff}, transformasi, pemilik, biaya, dan intended use. Catat juga alasan fitur dimasukkan atau dikeluarkan, bukti validasi, hasil ablation atau importance, pemeriksaan leakage, stabilitas, risiko privacy/fairness, serta rencana monitoring.
...
\tightlist
\item
  scikit-learn --- Feature selection --- \url{https://scikit-learn.org/stable/modules/feature_selection.html}. Menilai relevansi fitur secara terukur.
\item
  scikit-learn --- Permutation importance --- \url{https://scikit-learn.org/stable/modules/permutation_importance.html}. Pentingnya fitur yang model-agnostik.
\item
  SHAP --- \url{https://shap.readthedocs.io/}. Atribusi kontribusi fitur berbasis nilai Shapley.
\end{itemize}
